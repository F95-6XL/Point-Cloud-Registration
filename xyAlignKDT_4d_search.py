# Alignment of scans using a KDT and numpy.
#
# (c) 01 NOV 2020 Claus Brenner
#
import numpy as np
import matplotlib.pyplot as plt
from scipy.spatial import cKDTree
import open3d as o3d
import glob
import natsort
import time
from functions import  Hough_space_accumulator, check_intergrity, downsample, get_max_cons_matrix, rotate_points, calc_covarince_score_matrix, calc_rotated_points, find_matched_points, ICP_o3d, ICP_normal_vectors, ICP_normal_vectors2
from MaxConsMatrix import MaxConsMatrix
import h5py
from tqdm import tqdm



# Note the columns in the numpy scan files are:
# ("world_x", "world_y", "world_z", "nx", "ny", "nz",
#  "scan_id", "segment_id", "grid_xy", "reflectance")

# Hack: helper to draw a "crosshair" to a matrix, using NaN's.
def plot_crosshair(m):
    s = m.shape[0]//2
    m[s,:s-20] = np.nan
    m[s,s+20:] = np.nan
    m[:s-20,s] = np.nan
    m[s+20:,s] = np.nan
    return m

# Compute max cons alignment for two scans.
def align_two_scans(tree_0,points_0, points_1,
        max_cons_radius, max_cons_grid_edge, max_cons_z_range, max_cos_grid_angle, 
        max_cons_heading_range, rotation_center, count, use_normal_vectors=True, search_model = 'xy'):
    
    save_matrix = True
    save_cov_matrix = True
    save_matched_points = False
    save_hough_matrix = False
    
    # Trick for kd tree:
    # - In x and y, we want to find all points within distance
    #   +/- max_cons_radius.
    # - However, in z, we want to find points only within max_cons_z_range.
    # In order to have the same search radius in all 3 dimenstions, we
    # simply scale up z.
    z_scale = max_cons_radius / max_cons_z_range
    # points_0[:,2] *= z_scale
    points_1[:,2] *= z_scale
    
    # Calculate matrix_radius and heading_search_radius
    # The max cons matrix will be from -matrix_radius to +matrix_radius,
    # which will be 2*matrix_radius+1 elements.
    # Same by heading
    matrix_radius = int(np.ceil(max_cons_radius / max_cons_grid_edge))
    matrix_size = 2*matrix_radius+1
    heading_search_radius = int(np.ceil(max_cons_heading_range / max_cos_grid_angle))
    heading_search_size = 2 * heading_search_radius + 1
    
    
    if search_model == 'xyheading' or search_model == 'xyzheading':
        # If heading search applies, generate an array including all rotated points
        # The columns of array after rotation are 
        # ("world_x", "world_y", "world_z", "nx", "ny", "nz", "rotation_idx")
        points_1 = calc_rotated_points(points_1, max_cons_heading_range, 
                                      max_cos_grid_angle, heading_search_size, rotation_center)

    
    # For each point in points_1, find all the neighbors.
    # Note we use Minkowski inf-norm, using p=inf.
    # Neighbors will be an array of len(points_1) lists, each list containing
    # the indices of all neighbors. Lists may be empty.
    neighbors = tree_0.query_ball_point(points_1[:,0:3], max_cons_radius,
                                        p=np.inf)

    # # Make a flat list of indices into points_1.
    repetitions = [len(e) for e in neighbors]
    idx_1 = np.repeat(np.arange(len(points_1)), repetitions)

    # Also make a flat list of indices into points_0.
    idx_0 = np.concatenate(neighbors).astype(int)
    # print("pairs: %7d" % idx_0.shape[0])
    
    # Compute all xy offsets.
    # Here we compute the shift of scan 1, relative to scan 0.
    if search_model == 'xyz' or search_model == 'xyzheading':
        offsets = points_1[:,0:3][idx_1] - points_0[:,0:3][idx_0]
    else:
        offsets = points_1[:,0:2][idx_1] - points_0[:,0:2][idx_0]      

    # If required, compute scalar products of normal vectors.
    if use_normal_vectors:
        nv_prod = np.sum(points_0[:,3:6][idx_0] * points_1[:,3:6][idx_1],
                          axis=1) #法向量xyz分量分别相乘，然后相加
        # Zero where it is negative.
        nv_prod[nv_prod < 0.0] = 0.0
    
    # Convert into bins and matrix indices.
    factor = matrix_radius / max_cons_radius
    indices = np.round(offsets * factor).astype(int) + matrix_radius
    if search_model == 'xyheading' or search_model == 'xyzheading':
        # If heading search applys, also append rotation index to indices list
        indices = np.concatenate((indices, points_1[:,6][idx_1][:,np.newaxis]),axis=1).astype(int)
    
    
    # Find matched points for each cell of max cons matrix
    # matched_points_idx includes points index and there corresponding matrix coordinate
    # The colume of matched_points is 
    # ("map_point_index", "scan_point_index", "matrix_x", "matrix_y", "matrix_z", "matrix_heading")
    # where map_point_index and scan_point_index are the index of matched map & scan points w.r.t t
    # and t is the discrete cell in search space, t =  (matrix_x, matrix_y, matrix_z, matrix_heading)
    matched_points_idx = np.concatenate((idx_0[:,np.newaxis], idx_1[:,np.newaxis]), axis=1)
    matched_points_idx = np.concatenate((matched_points_idx, indices), axis=1)
    
    # Make maximum consensus matrix and covariance score matrix 
    cov_matrix = calc_covarince_score_matrix(points_0, points_1, matched_points_idx, matrix_size, 
                                    heading_search_size, search_model)
    
    max_cons_matrix = get_max_cons_matrix(nv_prod, indices, matrix_size, heading_search_size, 
                              search_model, use_normal_vectors)
    
    # Save maximum consensus matrix
    if save_matrix:
        # Make max cons matrix and accumulate all results.
        if search_model == 'xy':
            np.savetxt("./Matrix/new_data/max_cons_matrix/max_cons_matrix_"+str(count).zfill(5)+".txt", max_cons_matrix)
        else:
            # with h5py.File("F:/Studium/Masterarbeit/Code/Matrix/new_data/max_cons_matrix/max_cons_matrix_"+str(count).zfill(5)+".hdf5", 'w') as f:
            with h5py.File("./Matrix/new_data/max_cons_matrix/max_cons_matrix_"+str(count).zfill(5)+".hdf5", 'w') as f:
                f.create_dataset("matrix", data=max_cons_matrix, compression="gzip", compression_opts=5)
    
    # Save matched points for each cell in maximum consensus matrix
    if save_matched_points:
        # Set to True to firstly sort matched_points_idx after their matrix coordinate
        if False:
            if search_model == 'xy':
                matched_points_idx = matched_points_idx[np.lexsort((indices[:,1],indices[:,0]))]
            elif search_model == 'xyzheading':
                matched_points_idx = matched_points_idx[np.lexsort((indices[:,3],indices[:,2],indices[:,1],indices[:,0]))]
            else:
                matched_points_idx = matched_points_idx[np.lexsort((indices[:,2],indices[:,1],indices[:,0]))]
        np.savetxt("F:/Studium/Masterarbeit/Code/Matched_points/new_data/matrix_matched_points_"+str(count).zfill(5)+".txt", matched_points_idx,fmt='%d')   
    
    
    if save_cov_matrix:
        if search_model == 'xy':
            np.savetxt("./Matrix/new_data/cov_matrix/cov_matrix_"+str(count).zfill(5)+".txt", cov_matrix)
        else:
            # with h5py.File("F:/Studium/Masterarbeit/Code/Matrix/new_data/cov_matrix/cov_matrix_"+str(count).zfill(5)+".hdf5", 'w') as f:
            with h5py.File("./Matrix/new_data/cov_matrix/cov_matrix_"+str(count).zfill(5)+".hdf5", 'w') as f:
                f.create_dataset("cov_matrix", data=cov_matrix, compression="gzip", compression_opts=5)
    
    Matrix1 = MaxConsMatrix(max_cons_matrix, max_cons_radius, max_cons_grid_edge, 
                            matrix_size, max_cons_heading_range, max_cos_grid_angle, 
                            heading_search_radius, search_model, z_scale)
    Matrix2 = MaxConsMatrix(cov_matrix, max_cons_radius, max_cons_grid_edge, 
                        matrix_size, max_cons_heading_range, max_cos_grid_angle, 
                        heading_search_radius, search_model, z_scale)
    Matrix1.calc_shift()
    Matrix2.calc_shift()
    
    if save_hough_matrix:
        if search_model == 'xy':
            hough_matrix = Hough_space_accumulator(points_0, points_1, matched_points_idx, matrix_size, heading_search_size, search_model)
            a,b = Matrix1.highest_point_coordinate
            
            np.savetxt("F:/Studium/Masterarbeit/Code/Matrix/new_data/hough_matrix/max_cons_matrix/hough_matrix_"+str(count).zfill(5)+".txt", hough_matrix[a,b])
            c,d = Matrix2.highest_point_coordinate
            np.savetxt("F:/Studium/Masterarbeit/Code/Matrix/new_data/hough_matrix/cov_matrix/hough_matrix_"+str(count).zfill(5)+".txt", hough_matrix[c,d])
        else:
            hough_matrix = Hough_space_accumulator(points_0, points_1, matched_points_idx, matrix_size, heading_search_size, search_model)
            a,b,c = Matrix1.highest_point_coordinate
            np.savetxt("F:/Studium/Masterarbeit/Code/Matrix/new_data/hough_matrix/max_cons_matrix/hough_matrix_"+str(count).zfill(5)+".txt", hough_matrix[a,b,c])
            d,e,f = Matrix2.highest_point_coordinate
            np.savetxt("F:/Studium/Masterarbeit/Code/Matrix/new_data/hough_matrix/cov_matrix/hough_matrix_"+str(count).zfill(5)+".txt", hough_matrix[d,e,f])

    # Check intergrity 
    # intergrity = check_intergrity(c,Matrix2.shift, Matrix2.highest_point_coordinate, sigma = 1)
    
    
    # Do ICP based on outcomes of maximum consensus
    # Use points of matched scan points and whole map point cloud
    matched_map_points_unique, matched_scan_points_unique, _, _ = Matrix2.find_matched_points(points_0, points_1, matched_points_idx[:,0],
                                                              matched_points_idx[:,1],matched_points_idx[:,2:5], unique=True)

    results_icp2 = ICP_normal_vectors2(tree_0, matched_scan_points_unique, points_0.copy(),
                                      -Matrix2.shift, z_scale, search_model, rotation_center, use_weight=True)
    b = Matrix2.shift[0:2] - results_icp2[0:2].flatten()
    print(Matrix2.shift[0:3])
    
    return Matrix1.shift[0:3], Matrix2.shift[0:3], b
    
    
# ------
# Main
# ------
if __name__ == "__main__":
    
    
    time1_begin = time.time()
        
    path = 'D:\Masterarbeit_D\Data_D\Measurement\paper\*.txt' 
    files = glob.glob(path) 
        
    # Read .ply file
    pcl_map_load = o3d.io.read_point_cloud("D:/Masterarbeit_D/Data_D/Map/new/Merged_122831_122944_123114.ply")


    # Angle to select normal vectors. This is used to delete ground points.
    up_vector_angle =10.0
    cos_ub = np.cos(np.deg2rad(up_vector_angle))
    cos_lb = np.cos(np.deg2rad(180.0 - up_vector_angle))

    # Radius of max cons search. Result will be from -radius to +radius.
    max_cons_radius = 1

    # The grid size of the max cons grid.
    max_cons_grid_edge = 0.02

    # In this implementation, z is aligned already. When aligning x/y,
    # we will search only for corresponting points in a range +/- zrange.
    max_cons_z_range = 0.05
    
    # Angle and grid size of max cons search if search of heading applies
    max_cons_heading_range = 1
    max_cos_grid_angle = 0.25
    
    # Search model, default model is 'xy'
    # xy: max cons search in xy plane
    # xyz: max cons search in xyz space
    # xyheading: max cons search in xy plane including rotation alone z axis
    # xyzheading: max cons search in xyz space including rotation alone z axis
    search_model = 'xyheading'

    # Whether to weight the counts using the normal vector scalar product.
    use_normal_vectors = True
    
    # Remove ground
    remove_ground = True
    
    # Downsample
    Downsample = False
    voxel_size = 0.2
    
    # if search in z direction applies, set z_range equal to radius of max cons search
    if search_model == 'xyz' or search_model == 'xyzheading':
        max_cons_z_range = max_cons_radius
    
    # if search in heading direction applies, load rotation center
    rotation_center = np.loadtxt('D:/Masterarbeit_D/Data_D/Measurement/rotation_center/rotation_center_paper.txt', delimiter=",")
    rotation_center = rotation_center[:,2:5]
    
    if True:
        # Convert open3d format to numpy array
        # Here, you have the point cloud in numpy format. 
        pcl_map_load.estimate_normals(search_param = o3d.geometry.KDTreeSearchParamHybrid(radius=0.2, max_nn=100))
        
        pcl_map_numpy = np.asarray(pcl_map_load.points)
        pcl_map_numpy_normals = np.asarray(pcl_map_load.normals)
        map_pcl=np.concatenate((pcl_map_numpy, pcl_map_numpy_normals),axis=1)
        
        h_map_pcl = map_pcl[np.logical_and(map_pcl[:,5] >= cos_lb, map_pcl[:,5] <= cos_ub)]
        h_map_pcl = h_map_pcl[h_map_pcl[:,2]>97.8]
        
        # Set to true to estimate normal vector again with a second scale
        if False:
            pcl_map = o3d.geometry.PointCloud()
            pcl_map.points = o3d.utility.Vector3dVector(h_map_pcl[:,0:3])
            pcl_map.estimate_normals(search_param = o3d.geometry.KDTreeSearchParamHybrid(radius=0.6, max_nn=50000))
            pcl_map_numpy = np.asarray(pcl_map.points)
            pcl_map_numpy_normals = np.asarray(pcl_map.normals)
            map_pcl=np.concatenate((pcl_map_numpy, pcl_map_numpy_normals),axis=1)
            h_map_pcl = map_pcl[np.logical_and(map_pcl[:,5] >= cos_lb, map_pcl[:,5] <= cos_ub)]
    
    
        # Trick for kd tree:
        # - In x and y, we want to find all points within distance
        #   +/- max_cons_radius.
        # - However, in z, we want to find points only within max_cons_z_range.
        # In order to have the same search radius in all 3 dimenstions, we
        # simply scale up z.
        z_scale = max_cons_radius / max_cons_z_range
        h_map_pcl[:,2] *= z_scale
        
        tree_ = cKDTree(h_map_pcl[:,0:3])

    #o3d.open3d.visualization.draw_geometries([pcl_map_load],point_show_normal=True)
    files=natsort.natsorted(files)
    
    for i in range(1):
        # max_cons_grid_edge = 0.01 * i + 0.01
        count = 0
        score = 0
        eccentricity = []
        intergrity = 0
        avalability = []
        head = 0
        
        
        RS = np.empty((0,3))
        RS1 = np.empty((0,3))
        RS2 = np.empty((0,2))
        if search_model == 'xy':
            max_cons_shift = np.empty((0, 3))
        elif search_model == 'xyzheading':
            max_cons_shift = np.empty((0, 5))
        else:
            max_cons_shift = np.empty((0, 4))
        time3_begin_array = np.array([])
        time3_end_array = np.array([])
        tt=[]
        time2_begin = time.time()
        for name in tqdm(range(len(files))): 
            if count >= 100:
                count += 1
                continue
            points = np.loadtxt(files[name], delimiter=",")#'D:/Masterarbeit_D/Data_D/Measurement/Velodyne_33_global_without_outlier/01001_global.txt', delimiter=",")#
            
            points_pcd = o3d.geometry.PointCloud()
            points_pcd.points = o3d.utility.Vector3dVector(points)
            points_pcd.estimate_normals(
            search_param=o3d.geometry.KDTreeSearchParamHybrid(radius=0.7, max_nn=10000))
        #o3d.io.write_point_cloud("sync.ply", pcd)
        #pcd_load = o3d.io.read_point_cloud("sync.ply")
        
        # Convert Open3D.o3d.geometry.PointCloud to numpy array
            points_numpy = np.asarray(points_pcd.points)
            normals_numpy = np.asarray(points_pcd.normals)
            points=np.concatenate((points_numpy, normals_numpy),axis=1)
        
        # Vertical surfaces = horizontal normals.
            if remove_ground:
                h_points = points[np.logical_and(
                    points[:,5] >= cos_lb, points[:,5] <= cos_ub)]
                # h_points = h_points[h_points[:,2]>98]
  #               print("  remaining points on vertical surfaces", len(h_points))
            else:
                h_points=points
            #o3d.open3d.visualization.draw_geometries([points_pcd],point_show_normal=True)
            
        # Use downsampled points if needed
            if Downsample:
                hd_points = downsample(h_points, voxel_size, mode='NN')
            else:
                hd_points = h_points
                
            print('count:' + str(count))
            # Set to True to get a list of all scan id's in this tile.
            if False:
                # scan_id is column 6.
                scan_ids, counts = np.unique(h_points[:,6], return_counts=True)
                print("  sid: count")
                for scan_id, point_count in zip(scan_ids, counts):
                    print("  %3d: %5d" % (scan_id, point_count))
        
            time3_begin = time.time()#datetime.now()
            res_mc, rec_cov, res_icp= align_two_scans(tree_,h_map_pcl,hd_points, 
                                 max_cons_radius, max_cons_grid_edge, max_cons_z_range,
                                 max_cos_grid_angle, max_cons_heading_range, rotation_center[name],
                                 count, use_normal_vectors, search_model)
            
            time3_end = time.time()
            time3_begin_array = np.append(time3_begin_array, [time3_begin])
            time3_end_array = np.append(time3_end_array, [time3_end])
            # RS = np.concatenate((RS,a[np.newaxis,:]),axis=0)

            # iNTERGRITY
            # eccentricity.append(inte[0])
            # intergrity += inte[1]
            # avalability.append(inte[2])
            # head += inte[3]
            
            RS = np.concatenate((RS,res_mc[np.newaxis,:]),axis=0)
            RS1 = np.concatenate((RS1,rec_cov[np.newaxis,:]),axis=0)
            RS2 = np.concatenate((RS2,res_icp[np.newaxis,:]),axis=0)
 
            #max_cons_shift = np.append(max_cons_shift, [shift], axis=0)
            # if search_model == 'xy':
            #     plt.imshow(plot_crosshair(mc), interpolation="nearest")
            #     if use_normal_vectors:
            #         plt.savefig("./Results/Velodyne_with_nv/LOD_3/img_"+str(count)+"_nv"+".png")
            #     else: 
            #         plt.savefig("./Results/Velodyne/LOD_3/img_"+str(count)+".png")
            # if count == 1:
            #     break
            count += 1
            

        # print('score: '+str(score))
        # time1: run time for whole progamm
        # time2: run time for one run through all scan files
        # time3: run time for one "align_two_scans" function run
        time3 = np.sum(time3_end_array - time3_begin_array) / len(time3_end_array)
        time2_end = time.time()
        time2 = time2_end - time2_begin
        
        time1_end = time.time()
        time1 = time1_end - time1_begin
        # np.savetxt('cov_shift_lod3_0.04_downsample0.2_ICP_normal_vector_wholemap.txt', RS)
        # np.savetxt('shift_lod3_0.04_downsample0.2.txt', RS1)
        # np.savetxt('cov_shift_lod3_0.04_downsample0.2.txt', RS2)
        # np.savetxt('shift_lod3.txt', max_cons_shift)
        print('time1: ' + str(time1) + '\n' + 'time2 = ' + str(time2) +'\n' + 'time3 = ' + str(time3))
    # np.savetxt('time.txt',time0)
    # Finally, call show.
    plt.show()
